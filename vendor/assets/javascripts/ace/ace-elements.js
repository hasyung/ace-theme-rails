(function($ , undefined) {
  var multiplible = 'multiple' in document.createElement('INPUT');
    ...
    this.each(function(){
      ...
      var wrap_class = $this.attr('data-wrap-class') ? " "+$this.attr('data-wrap-class') : "";


      $this.wrap("<div class='ace-file-input"+wrap_class+(well_style ? " ace-file-multiple":"")+"' />");
      ...
      if(settings.droppable && hasFileList) {
        ...
        $(dropbox).on('dragenter', function(e){
          ...
          var ret = true;
          if(settings.before_change) ret = settings.before_change.call(that, $this, files, true);//true means files have been dropped
          if(!ret || ret.length == 0) {
            return false;
          }
      ...
      $this.on('change.inner_call', function(e , inner_call){
        ...
        var ret = true;
        if(settings.before_change) ret = settings.before_change.call(that, $this, this.files || this.value, false);//false means files have been selected, not dropped
        if(!ret || ret.length == 0) {
